...
public:
    ...
    void setParameters (double freqHz, double Rmult01)
    {
        constexpr double Rfb = 82.0e3f;
        const double R_g = 100.0f * std::pow (10000.0f / 100.0f, std::pow (Rmult01, 0.37f));
        double C = 1.0f / (2 * M_PI * freqHz * std::sqrt (Rfb * R_g));

        setParameters (Rfb, R_g, 6e-9);
    }
